...
class ResultParser:
    ...
    #_metrics = ['baseline', 'baseline', 'declare']
    ...
    def _print_score(score, std, reference_score, highlight_type, column_type):
        ...
        if highlight_type == ResultParser.HighlightTypes.ROW_SCORE:
            if score == reference_score:
                print('\\cellcolor[HTML]{%s}\\textbf{%.0f %.0f}' % (column_color, 100*score, 100*std)),
            else:
                print('%.0f %.0f' %(100*score, 100*std)),
    ...
    def _load_table(self, folderpath):
        if folderpath == 'reference':
            ...
            return np.zeros((len(self._log_names), len(self._metrics) * len(self._model_types) * 2 - 2)),\
                   np.zeros((len(self._log_names), len(self._metrics) * len(self._model_types) * 2 - 2))
        else:
            table_folds = []
