...
from evaluation.prepare_data_resource import amplify, select_petrinet_verified_traces, \
...
def run_experiments(log_name, models_folder, fold):
    ...
    with open(output_filename, 'w') as csvfile:
        ...
        csv_writer.writerow(["Prefix length", "Ground truth", "Predicted", "Damerau-Levenshtein", "Jaccard",
                             "Ground truth times", "Predicted times", "RMSE", "MAE", "Median AE",
                             "Ground Truth Resource", "Predicted Resource", "Damerau-Levenshtein Resource",
                             "compliantness"])
        ...
        for prefix_size in range(prefix_size_pred_from, prefix_size_pred_to):
            ...
            for line, line_id, line_group, times, times2, times3, times4 \
                ...
                search_node_root = NodePrediction(encode(cropped_line, cropped_line_group, cropped_times,
                                                         cropped_times3, maxlen, chars, chars_group, char_indices,
                                                         char_indices_group, divisor, divisor2),
                                                  cropped_line_id, cropped_line, cropped_line_group, cropped_times4,
                    ...
                    for k in range(current_beam_size):
                        ...
                        if not found_satisfying_constraint:
                            if verify_with_data(pn_model_filename, current_prediction_premis.trace_id,
                                ...
                                activity = map(lambda x: str(get_int_from_unicode(x)), current_prediction_premis.cropped_line[prefix_size:])
                                group = map(lambda x: str(get_int_from_unicode(x)), current_prediction_premis.cropped_line_group[prefix_size:])
                        ...
                        for j in range(current_beam_size):
                            ...
                            if temp_prediction == '!':
                                if verify_with_data(pn_model_filename, current_trace_id, temp_cropped_line_group,
                                                    temp_cropped_line_group, temp_cropped_times):
                                    stop_symbol_probability_amplifier_current = 1
                            ...
                            temp_state_data = encode(temp_cropped_line, temp_cropped_line_group, cropped_times,
                                                     cropped_times3, maxlen, chars, chars_group, char_indices,
                                                     char_indices_group, divisor, divisor2)

                            temp = NodePrediction(temp_state_data, cropped_line_id, temp_cropped_line,
                                                  temp_cropped_line_group, cropped_times4, temp_total_predicted_time,
                ...
                if len(ground_truth) > 0:
                    ...
                    dls_res = 1 - (damerau_levenshtein_distance(predicted_group, ground_truth_group) / max(
                        len(predicted_group), len(ground_truth_group)))
                    if dls_res < 0:
                        dls_res = 0
