...
from src.evaluation.prepare_data import prepare_testing_data, select_petrinet_verified_traces, get_symbol, encode, \
    get_pn_fitness
...
def run_experiments(log_name, models_folder, fold):
    ...
    folder_path = shared.output_folder / models_folder / str(fold) / 'results' / 'beamsearch'
                    ...
                    for k in range(current_beam_size):
                        ...
                        if not found_satisfying_constraint:
                            if get_pn_fitness(pn_model_filename, line_id, current_prediction_premis.cropped_line) \
                                    >= shared.log_settings[log_name]['th_evaluation']:
